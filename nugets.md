...
| FluentValidation.DependencyInjectionExtensions | 11.9.0 | Expone métodos de extensión para registrar validadores en el contenedor de servicios. | `AddApplication` llama a `services.AddValidatorsFromAssembly(Assembly.GetExecutingAssembly())` para descubrir validadores automáticamente.【F:Backend/ProyectoBase.Application/DependencyInjection.cs†L18-L29】 |
...
| AutoMapper.Extensions.Microsoft.DependencyInjection | 12.0.1 | Registra perfiles de AutoMapper en el contenedor de dependencias. | `AddApplication` invoca `services.AddAutoMapper(...)` para descubrir y registrar los perfiles declarados en la asamblea.【F:Backend/ProyectoBase.Application/DependencyInjection.cs†L30-L33】 |
| Microsoft.Extensions.Caching.Abstractions | 8.0.8 | Proporciona contratos de caché (memoria/distribuida) reutilizables. | `GetAllProductsQueryHandler` utiliza `IDistributedCache` para reutilizar resultados serializados y evitar consultas redundantes al repositorio.【F:Backend/ProyectoBase.Application/Products/Queries/GetAllProducts/GetAllProductsQueryHandler.cs†L25-L76】 |
| Microsoft.Extensions.Logging.Abstractions | 8.0.1 | Ofrece contratos comunes para generar logs desde la capa de aplicación. | `GetAllProductsQueryHandler` registra errores inesperados con `ILogger<GetAllProductsQueryHandler>` cuando falla el acceso a datos o caché.【F:Backend/ProyectoBase.Application/Products/Queries/GetAllProducts/GetAllProductsQueryHandler.cs†L26-L85】 |
...
| Microsoft.Extensions.Logging.Abstractions | 8.0.1 | Expone interfaces de logging consumidas por los servicios de infraestructura. | `DependencyInjection` obtiene `ILoggerFactory` para emitir advertencias y errores al configurar las políticas de resiliencia utilizadas por los repositorios.【F:Backend/ProyectoBase.Infrastructure/DependencyInjection.cs†L63-L108】 |
...
| Paquete | Versión | Propósito | Ejemplo en el código |
| --- | --- | --- | --- |
| FluentValidation | 11.9.0 | Proporciona el helper `TestValidate` y reglas compartidas para validar DTOs en pruebas. | `CreateProductDtoValidatorTests` usa la extensión `TestValidate` para verificar reglas de validación en los DTOs de producto.【F:Backend/ProyectoBase.Application.Tests/Validators/CreateProductDtoValidatorTests.cs†L15-L58】 |
